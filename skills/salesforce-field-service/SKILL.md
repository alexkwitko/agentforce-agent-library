...
- **Two consoles**: Classic Dispatch Console (Aura/VF Gantt, works out of the box) vs the new **Scheduling Console** (LWC) which **requires ES&O** (`o2EngineEnabled` — a significant ~one-way engine migration; enable via the guided Setup flow). See dispatcher-console-permissions.md §0.
- **Empty dispatcher map** = territories aren't geocoded. Set `ServiceTerritory` `StateCode`/`CountryCode` (NOT State/Country text — picklist orgs throw FIELD_INTEGRITY) + `Latitude`/`Longitude` (directly writable) + resource `LastKnownLatitude/Longitude`. Boundary polygons (`FSL__Polygon__c`) are drawn in the Map UI.
