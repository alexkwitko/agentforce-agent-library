...
- **⚠️ "We couldn't load the availability" / "couldn't check rule violations" in the new Scheduling Console — checklist (in priority order):**
  ...
  - **If it STILL errors after all the above** (data fully correct + Classic console works on the same data): it's an **ES&O availability-service provisioning issue**, not your config. Enabling `o2EngineEnabled` flips the flag but the ES&O availability engine can need additional/async provisioning that doesn't always complete in a **Developer Edition** org. The **`sfdc_fieldservice` ("Field Service Integration") perm set canNOT be assigned to a regular admin user** ("license doesn't match") — it's for the integration user, so that's not the fix. **Fallback: use the Classic Dispatch Console** (it uses the classic engine and works on the same data); the new console's availability typically needs Salesforce-side ES&O provisioning to finish.
